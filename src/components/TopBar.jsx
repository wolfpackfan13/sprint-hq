...
import { Sparkles, ChevronDown, Check, Pencil } from 'lucide-react'

export function TopBar({ sprint, currentWeek, progress, vision, onSaveVision, onEditSprint, companies, activeClient, onSelectClient }) {
  ...
  return (
    <div className="bg-white border-b border-surface-300 flex-shrink-0 relative z-20">
      {/* Single line: progress + why toggle + client filter */}
      <div className="px-4 py-2 flex items-center gap-3">
        {/* Sprint progress sliver */}
        {sprint && (
        ...
        {/* Why toggle */}
        <button onClick={() => setWhyOpen(o => !o)} className="flex items-center gap-1 text-xs text-navy-500 hover:text-gold-600 transition-colors min-w-0">
          <Sparkles size={12} className="text-gold-500 flex-shrink-0" />
          <span className="font-display font-semibold flex-shrink-0">My Why</span>
        ...
        <div className="relative flex-shrink-0">
          <button onClick={() => setClientOpen(o => !o)}
            ...
            {activeCo ? <>{activeCo.emoji} {activeCo.name}</> : 'All Clients'}
          ...
          {clientOpen && (
            <>
              ...
              <div className="absolute right-0 top-full mt-1 bg-white border border-surface-300 rounded-xl shadow-modal py-1 z-40 min-w-[180px]">
                <button onClick={() => { onSelectClient(null); setClientOpen(false) }}
                  className="w-full flex items-center justify-between px-3 py-2 text-sm text-navy-700 hover:bg-surface-100">
                  All Clients {!activeClient && <Check size={14} className="text-gold-600" />}
                </button>
                {companies.map(co => (
                  <button key={co.id} onClick={() => { onSelectClient(co.id); setClientOpen(false) }}
                    className="w-full flex items-center justify-between px-3 py-2 text-sm hover:bg-surface-100">
          ...
          {editingWhy ? (
            <div className="pt-2">
              <textarea value={draft} onChange={e => setDraft(e.target.value)} rows={2} autoFocus
                placeholder="I'm building toward..."
